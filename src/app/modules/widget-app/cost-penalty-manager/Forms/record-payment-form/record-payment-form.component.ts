...
} from './record-payment-charge-form.model';
...
@Component({
  ...
  styleUrls: ['./record-payment-form.component.css'],
})
export class RecordPaymentFormComponent
  extends WidgetComponentBase
  implements OnInit
{
  @Input() dataModel: RecordPaymentChargeFormModel;
  @Input() configModel: RecordPaymentChargeFormConfigModel;

  RecordPaymentChargeForm: FormGroup;
  ...
  OnDestroy() {
    this.RecordPaymentChargeForm.reset();
  }
  createFormGroup(): void {
    this.RecordPaymentChargeForm = this.fb.group({
  ...
  getValue() {
    ...
    if (this.RecordPaymentChargeForm.valid) {
      return this.RecordPaymentChargeForm.value;
    }
  }
  ...
  openFileUpload() {
    ...
    this.emitEvent('openFileUploadRecordPaymentDialog', data);
  }
}
