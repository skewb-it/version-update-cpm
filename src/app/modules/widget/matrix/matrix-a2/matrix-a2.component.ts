...
import { MatrixA2ConfigModel, MatrixA2HeaderItemModel, MatrixA2ItemModel, MatrixA2Model, MatrixA2RowHeadingItemModel } from './matrix-a2-model';
...
// import { MatrixA2ConfigModel, MatrixA2HeaderItemModel, MatrixA2ItemModel, MatrixA2Model, MatrixA2RowHeadingItemModel } from './martrix-a2-model';
...
// import { CLICK_EVENT } from '../../widget-app/data-diff/data-diff-a3/data-diff-a3-model';


@Component({
  selector: 'app-matrix-a2',
  templateUrl: './matrix-a2.component.html',
  styleUrls: ['./matrix-a2.component.css']
})
export class MatrixA2Component extends WidgetComponentBase implements OnInit {
  @Input() dataModel: MatrixA2Model;
  @Input() configModel: MatrixA2ConfigModel;
  ...
  getHeaderItem(colNumber: number) {
    ...
    return new MatrixA2HeaderItemModel();
  }
  ...
  getRowHeaderItem(rowNumber: number) {
    ...
    return new MatrixA2RowHeadingItemModel();
  ...
  checkfiltervaluetotoal(rowNumber: number, colNumber: number) {
    try {
      if(this.dataModel.items[rowNumber][colNumber].filterValue==MANAGEMENT_DB_MATRIX_FILTER.REGISTRATIONS_TOTAL || this.dataModel.items[rowNumber][colNumber].filterValue==MANAGEMENT_DB_MATRIX_FILTER.DEFECTS_TOTAL || this.dataModel.items[rowNumber][colNumber].filterValue==MANAGEMENT_DB_MATRIX_FILTER.FPNS_TOTAL){
  ...
  getRatioWidth(rowNumber: number, colNumber: number,index) {
    ...
    let totalWidthInPercentage = this.dataModel.items[rowNumber][colNumber].ratioChartCount1 + this.dataModel.items[rowNumber][colNumber].ratioChartCount2;
    switch (index) {
      case 1:
        widthInPercentage = (this.dataModel.items[rowNumber][colNumber].ratioChartCount1 / totalWidthInPercentage) * 100;
        break;

      case 2:
        widthInPercentage = (this.dataModel.items[rowNumber][colNumber].ratioChartCount2 / totalWidthInPercentage) * 100;
        break;
  ...
  onClick(rowIndex, columnIndex, itemsData:MatrixA2ItemModel, rowData: MatrixA2RowHeadingItemModel, columnData: MatrixA2HeaderItemModel) {
  ...
  matrixConvertData(response) {
    console.log("matrix a2 response",response)
