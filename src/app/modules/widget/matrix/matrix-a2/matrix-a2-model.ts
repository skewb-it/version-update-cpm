...
export class MatrixA2ConfigModel {

  static getInstance<T>(): MatrixA2ConfigModel {
    let model = new MatrixA2ConfigModel();
    return model
  }
...
export class MatrixA2Model {

  static getInstance<T>(): MatrixA2Model {
  let model = new MatrixA2Model();
  ...
  static getEmptyItems(rows: number, columns: number): MatrixA2ItemModel[][] {
    ...
    for (let i = 0; i < rows; i++) {
      ...
      for (let j = 0; j < columns; j++) {
        let itemModel = new MatrixA2ItemModel();
  ...
  headers: MatrixA2HeaderItemModel[]
  rowHeaders: MatrixA2RowHeadingItemModel[]
  items: MatrixA2ItemModel[][];
...
export class MatrixA2HeaderItemModel {
    constructor() {
        this.headerText = '';
    }
    headerText: string;
...
export class MatrixA2RowHeadingItemModel {
...
export class MatrixA2ItemModel {
  displayText?: number;
  filterValue?:string;
  percentage?: number;
  bottonText?: number;
