...
import { CreatePermitSummuryConfigModel, CreatePermitSummuryDataModel } from '../../widget-app/create-permit-summary/create-permit-summury.model';
...
import { GLOBAL_PARAM_KEY, JOURNEY_TYPE, NAVIGATION_DATA } from 'src/app/app-constants';
...
import { MapPageDataDataModel, NSGDetails, PermitConditionsPageModel, PermitFormPageData, PermitNavigationDataModel, SpecialDesignationsModel } from './permit-listing.model';
...
import { CreatePermitSelectLocationComponent } from '../../widget-app/create-permit-select-location/create-permit-select-location.component';
...
@Component({
  selector: 'app-permit-listing',
  templateUrl: './permit-listing.component.html',
  styleUrls: ['./permit-listing.component.css'],
})

// container/page/parent
export class PermitListingComponent extends WidgetPageBase implements OnInit {
  ...
  @ViewChild(CreatePermitDrawMapComponent)
  createPermitDrawMapComponent: CreatePermitDrawMapComponent;
  @ViewChild(CreatePermitFormComponent)
  createPermitFormComponent: CreatePermitFormComponent;
  @ViewChild(PermitConditionsTabComponent)
  permitConditionsTabComponent: PermitConditionsTabComponent;
  @ViewChild(CreatePermitSummaryComponent)
  createPermitSummaryComponent: CreatePermitSummaryComponent;
  @ViewChild(PermitImagesAndFilesTabComponent)
  permitImagesAndFilesTabComponent: PermitImagesAndFilesTabComponent;

  @ViewChild(GlobalFilterComponent)
  globalFilterComponent: GlobalFilterComponent;
  ...
  @ViewChild(PermitQuickFilterComponent)
  permitQuickFilter: PermitQuickFilterComponent;
  ...
  arrCreatePermitDialogTitle = [
    'Create Permit - Draw Map',
    'Create Permit',
    'Create Permit - Select Conditions',
    'Upload Images & Files',
    'Check Permit Details Before Submission',
    'Permit Request Confirmed',
  ];
  arrEditPermitDialogTitle = [
    'Edit Permit - Draw Map',
    'Edit Permit',
    'Edit Permit - Select Conditions',
    'Upload Images & Files',
    'Check Permit Details Before Submission',
    'Permit Request Confirmed',
  ];
  ...
  constructor(
    ...
    this.permitConfirmationDataModel =
      PermitConfirmationDataModel.getInstance();
    this.permitConfirmationConfigModel =
      PermitConfirmationConfigModel.getInstance();
    ...
    this.permitQuickFilterConfigModel =
      PermitQuickFilterConfigModel.getInstance();
    ...
    this.permitConditionsConfigModel =
      PermitConditionsConfigModel.getInstance();

    this.permitImagesAndFilesDataModel =
      PermitImagesAndFilesModel.getInstance();
    this.permitImagesAndFilesConfigModel =
      PermitImagesAndFilesConfigModel.getInstance();
  ...
  checkAndBindDynamicFilterData(shouldCompulsoryRefreshList) {
    ...
    if (
      ...
      this.permitListDataModel.apireqdata = {
        ...filterGlobalPersistData.persistantDynamicFilterModel,
      };
      this.permitList.resetPagination();
      this.permitListConfigModel.CallerToComp.emit(
  ...
  bindPermitListEvents() {
    ...
    super.addEventListener(
      eventActions,
      this.permitListDataModel,
      this.permitListConfigModel
    );
  }
  ...
  bindPermitFilterEvents() {
    ...
    super.addEventListener(
      eventActions,
      this.permitFilterDataModel,
      this.permitFilterConfigModel
    );
  }
  ...
  bindElginMapEvents() {
    ...
    super.addEventListener(
      eventActions,
      this.elginMapDataModel,
      this.elginMapConfigModel
    );
  }
  ...
  bindCreatePermitFormEvents() {
    ...
    super.addEventListener(
      eventActions,
      this.createPermitFormDataModel,
      this.createPermitFormConfigModel
    );
  }
  ...
  bindGlobalFilterConfigModel() {
    ...
    this.addEventListener(
      events,
      this.globalFilterDataModel,
      this.globalFilterConfigModel
    );
  }

  page_drawMapOverlayComplete(eventparams, wigDataContext, params) {
    this.permitDrawMapConfigModel.CallerToComp.emit(
      WidgetConstants.LOAD_DATA,
      eventparams
    );
  }
  ...
  openDialog() {
    ...
    let permitFormPageData: PermitFormPageData = this.globalParameters.get(
      GLOBAL_PARAM_KEY.PERMIT_FORM
    );
    permitFormPageData = permitFormPageData
      ? permitFormPageData
      : new PermitFormPageData();
    ...
    this.dialogModel.header =
      this.arrCreatePermitDialogTitle[this.createPermitStepNo - 1];
  ...
  nextButtonDrawMapClick() {
    if (this.createPermitDrawMapComponent.validate()) {
      // map page details
      let mapPageDataDataModel: MapPageDataDataModel =
        new MapPageDataDataModel();
      mapPageDataDataModel.geometry = this.elginmapComponent.getValue();
      mapPageDataDataModel.nsgDetails =
        this.createPermitDrawMapComponent.getValue();
      this.globalParameters.set(
        GLOBAL_PARAM_KEY.ELGIN_MAP,
        mapPageDataDataModel
      );
      ...
      permitFormPageData.geometry = {
        type: ElginMapUtility.getGeometryTypeForRequest(
          mapPageDataDataModel.geometry.type
        ),
        coordinates: mapPageDataDataModel.geometry.cordinateNorthingsEastings,
      };

      let arrHighWayAuthorities = this._appRepoHelperService.getMDataByDomain(
        MASTER_DATA_DOMAIN.HIGHWAY_AUTORITIES.toString()
      );
      arrHighWayAuthorities.forEach((authority: any) => {
        if (
          authority.swaCode ==
          mapPageDataDataModel.nsgDetails.authority_swa_code
        ) {
          permitFormPageData.highwayAuthorityId = authority.authorityId;
        }
      });

      permitFormPageData.locationDescription =
        mapPageDataDataModel.nsgDetails.worksLocationDescription;
      permitFormPageData.locationDetails =
        mapPageDataDataModel.nsgDetails.location_details;
      ...
      permitFormPageData.roadCategory =
        mapPageDataDataModel.nsgDetails.road_category;
      permitFormPageData.roadType = mapPageDataDataModel.nsgDetails.roadType;
      permitFormPageData.streetName =
        mapPageDataDataModel.nsgDetails.street_name;
      permitFormPageData.town = mapPageDataDataModel.nsgDetails.town;
      permitFormPageData.specialDesignations = [];
      mapPageDataDataModel.nsgDetails.special_designations.forEach(
        (designation) => {
          if (designation.isChecked) {
            let specialDesignation = {
              ...
              special_desig_location_text:
                designation.special_desig_location_text,
              ...
              special_desig_periodicity_code:
                designation.special_desig_periodicity_code,
            };
            permitFormPageData.specialDesignations.push(specialDesignation);
          }
        }
      );
      permitFormPageData.trafficSensitiveFlag =
        mapPageDataDataModel.nsgDetails.traffic_sensitive;

      this.globalParameters.set(
        GLOBAL_PARAM_KEY.PERMIT_FORM,
        permitFormPageData
      );
      this.createPermitFormDataModel.globalParameters.set(
        GLOBAL_PARAM_KEY.PERMIT_FORM,
  ...
  nextButtonPermitFormClick() {
    ...
    this.dialogModel.header = this.isCreatePermitModeCreate
      ? this.arrCreatePermitDialogTitle[this.createPermitStepNo - 1]
  ...
  nextButtonConditionClick() {
    ...
    if (applicationId) {
      this._serverApiBase
        .put<any, any>(`/api/v1/applications/${applicationId}`, data)
        .subscribe(
          (response) => {
            try {
      ...
      this._serverApiBase
        .post<any, any>('/api/v1/applications', data)
        .subscribe(
          (response) => {
            try {
              if (response && response.applicationId) {
  ...
  backButtonConditionClick() {
    ...
    let conditions: PermitConditionsPageModel[] =
      this.permitConditionsTabComponent.getValue();
  ...
  setPermitFormDataToPageData() {
    ...
    // geometry
    let mapPageDataDataModel: MapPageDataDataModel = this.globalParameters.get(
      GLOBAL_PARAM_KEY.ELGIN_MAP
    );
    permitFormPageData.geometry = {
      type: ElginMapUtility.getGeometryTypeForRequest(
    ...
    permitFormPageData.workReferenceNumber =
      permitFormData.work_reference_number;
    ...
    permitFormPageData.locationDescription =
      permitFormData.location_description;
    ...
    permitFormPageData.workDescTextType =
      permitFormData.work_description_text_type;
    permitFormPageData.activityType = permitFormData.activity_type;
    permitFormPageData.trafficManagementType =
      permitFormData.traffic_management_type;
    permitFormPageData.closeFootway = permitFormData.close_footway;
    permitFormPageData.collaborativeWorkingFlag =
      permitFormData.collaborative_working_flag;
    permitFormPageData.collaborationDetails =
      permitFormData.collaboration_details;
    ...
    permitFormPageData.preApprovalAuthoriser =
      permitFormData.pre_approval_authoriser;
  ...
  setConditionsToGlobalParameters() {
    let conditionsData: PermitConditionsPageModel =
      this.permitConditionsTabComponent.getValue();
    conditionsData = conditionsData
      ? conditionsData
      : new PermitConditionsPageModel();
  ...
  getPermitRequestBody() {
    let data: any = {
      ...
      geometry: {
        ...
        coordinates: [
          [
            [503294.847231123, 192583.873611111],
            [503280.147220442, 192531.140275998],
            [503286.058330366, 192530.440275998],
            [503301.613890669, 192581.462495846],
            [503294.847231123, 192583.873611111],
          ],
        ],
      },
    ...
    data.geometry.type = permitFormPageData.geometry.type
      ? permitFormPageData.geometry.type
      : [];
    data.geometry.coordinates = permitFormPageData.geometry.coordinates
      ? permitFormPageData.geometry.coordinates
      : [];
    data.location_description = permitFormPageData.locationDescription
      ? permitFormPageData.locationDescription
      : null;
    data.location_details = permitFormPageData.locationDetails
      ? permitFormPageData.locationDetails
      : null;
    data.highway_authority_id = permitFormPageData.highwayAuthorityId
      ? permitFormPageData.highwayAuthorityId
      : 0;
    data.usrn = permitFormPageData.usrn ? permitFormPageData.usrn : null;
    data.proposed_start_date = permitFormPageData.proposedStartDate
      ? permitFormPageData.proposedStartDate
      : null;
    data.proposed_end_date = permitFormPageData.proposedEndDate
      ? permitFormPageData.proposedEndDate
      : null;

    data.org_id = permitFormPageData.orgId ? permitFormPageData.orgId : 0;
    data.department_id = permitFormPageData.departmentId
      ? permitFormPageData.departmentId
      : 0;
    ...
    data.lane_rental_flag = permitFormPageData.laneRentalFlag
      ? permitFormPageData.laneRentalFlag
      : false;

    data.excavation_flag = permitFormPageData.excavationFlag
      ? permitFormPageData.excavationFlag
      : false;

    let conditions = this.globalParameters.get(
      GLOBAL_PARAM_KEY.CONDITIONS
    ).conditions;
    data.conditions = conditions ? conditions : [];

    data.special_designations = permitFormPageData.specialDesignations
      ? permitFormPageData.specialDesignations
      : [];
    ...
    data.work_description = permitFormPageData.workDescription
      ? permitFormPageData.workDescription
      : 0;

    data.work_category = permitFormPageData.workCategory
      ? permitFormPageData.workCategory
      : 0;

    data.postcode = permitFormPageData.postCode
      ? permitFormPageData.postCode
      : null;

    data.accountability = permitFormPageData.accountability
      ? permitFormPageData.accountability
      : 0;

    data.collaborative_working_flag =
      permitFormPageData.collaborativeWorkingFlag
        ? permitFormPageData.collaborativeWorkingFlag
        : false;
    ...
    data.environmental_flag = permitFormPageData.environmental_flag
      ? permitFormPageData.environmental_flag
      : false;
    data.is_covid19_response = permitFormPageData.is_covid19_response
      ? permitFormPageData.is_covid19_response
      : false;

    data.street_name = permitFormPageData.streetName
      ? permitFormPageData.streetName
      : '';
  ...
  permitConfirm() {
    ...
    if (applicationId) {
      this._serverApiBase
        .post<any, any>(
          '/api/v1/applications/' + applicationId + '/submission',
          null
        )
        .subscribe(
          (submitresp) => {
            try {
              if (submitresp && submitresp.permit_reference_number) {
                this.permitConfirmationDataModel.globalParameters.set(
                  'applicationId',
  ...
  confirmDelete() {
    this.showSpinner();
    let applicationIdForDelete = this.globalParameters.get(
      'applicationIdForDelete'
    );
    this._serverApiBase
      .delete<any, any>(`/api/v1/applications/${applicationIdForDelete}`)
      .subscribe(
        (response) => {
          try {
            if (response) {
  ...
  setPermitNavigationKey(mode) {
    ...
    this.permitNavigationKey =
      mode == FormModeConstant.ADD
  ...
  page_on_global_filter_change() {
    ...
    this._appRepoHelperService.setGlobalPersistantData(
