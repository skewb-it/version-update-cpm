import { AppFilterModel, PersistantAdvanceFilterModel, PersistantDynamicFilterModel } from 'src/app/models/common/app-filter';
import { CHANGE_REQUEST_DIALOG, MASTER_DATA_DOMAIN, PERMIT_CONFIRMATION_DIALOG, PERMIT_GEOMETRY_TYPE, REINSTATEMENT_DIALOG_MODE, SAVE_AS_DRAFT_POST_ACTION } from 'src/app/constants/app-repo.constants';
...
import { CreateReinstatementSummuryConfigModel, CreateReinstatementSummuryDataModel } from '../../widget-app/create-reinstatement-summary/create-reinstatement-summary.model';
import { DataDiffA3ConfigModel, DataDiffA3Model } from '../../widget-app/data-diff/data-diff-a3/data-diff-a3-model';
...
import { MapPageDataDataModel, NSGDetails, PermitConditionsPageModel, PermitFormPageData, PermitNavigationDataModel } from '../permit-listing/permit-listing.model';
import { PermitActivityLogConfigModel, PermitActivityLogModel } from '../../widget-app/permit-activity-log-tab/permit-activity-log.model';
import { PermitAddCommentConfigModel, PermitAddCommentDataModel } from '../../widget-app/permit-add-comment/add-comment.model';
import { PermitCancelConfirmConfigModel, PermitCancelConfirmDataModel } from '../../widget-app/cancel-permit-confirmation/cancel-permit-confirmation.model';
import { PermitChangeReqConfigModel, PermitChangeReqDataModel } from '../../widget-app/permit-change-request-tab/permit-change-request-tab.module';
import { PermitCommentsConfigModel, PermitCommentsDataModel } from '../../widget-app/permit-comments-tab/permit-comments-tab.module';
...
import { PermitFPNConfigModel, PermitFPNDataModel } from '../../widget-app/permit-fpn-tab/permit-fpn-tab.model';
...
import { PermitInspectionListConfigModel, PermitInspectionListDataModel } from '../../widget-app/permit-inspections-tab/permit-inspection-list.model';
import { PermitKeyDetailsConfigModel, PermitKeyDetailsModel } from '../../widget-app/permit-key-details/permit-key-details.model';
import { PermitMapConfigModel, PermitMapModel } from '../../widget-app/permit-map-tab/permit-map-tab.model';
import { PermitMapDesignationConfigModel, PermitMapDesignationModel } from '../../widget-app/permit-map-designation-tab/permit-map-designation-tab.model';
import { PermitOverviewConfigModel, PermitOverviewModel } from '../../widget-app/permit-overview-tab/permit-overview-tab.model';
import { PermitReinstatementConfigModel, PermitReinstatementDataModel } from '../../widget-app/permit-reinstatement-tab/permit-reinstatement-tab-modele';
import { PermitViewConfigModel, PermitViewDetailsModel, PermitViewItemsModel, PermitViewModel } from './permit-view.model';
import { RaiseChangeReqConfirmationConfigModel, RaiseChangeReqConfirmationDataModel } from '../../widget-app/raise-change-req-confirmation/raise-change-req-confirmation.model';
import { ReinstatementConfigModel, ReinstatementDataModel } from '../../widget-app/create-reinstatement-form/reinstatement.model';
import { ReinstatementImagesAndFilesConfigModel, ReinstatementImagesAndFilesDataModel, ReinstatementImagesAndFilesDetailsModel } from '../../widget-app/reinstatement-images-and-files/reinstatement-images-and-files.model';
import { SiteDetailsConfigModel, SiteDetailsDataModel } from '../../widget-app/site-details/site-details-model';
import { ViewPermitCommentConfigModel, ViewPermitCommentsDataModel } from '../../widget-app/view-permit-comments/view-permit-comments.model';
import { ViewPermitFpnConfigModel, ViewPermitFpnDataModel } from '../../widget-app/view-permit-fpn/view-permit-fpn.module';
import { ViewPermitInspectionConfigModel, ViewPermitInspectionDataModel } from '../../widget-app/view-permit-inspection/view-permit-inspection.model';
...
import { CreateReinstatementFormComponent } from '../../widget-app/create-reinstatement-form/create-reinstatement-form.component';
...
import { PermitCommentsTabComponent } from '../../widget-app/permit-comments-tab/permit-comments-tab.component';
import { PermitConditionsTabComponent } from '../../widget-app/permit-conditions-tab/permit-conditions-tab.component';
import { PermitFpnTabComponent } from '../../widget-app/permit-fpn-tab/permit-fpn-tab.component';
...
import { PermitInspectionsTabComponent } from '../../widget-app/permit-inspections-tab/permit-inspections-tab.component';
import { PermitMapDesignationTabComponent } from '../../widget-app/permit-map-designation-tab/permit-map-designation-tab.component';
import { PermitOverviewTabComponent } from '../../widget-app/permit-overview-tab/permit-overview-tab.component';
import { ReinstatementImagesAndFilesComponent } from '../../widget-app/reinstatement-images-and-files/reinstatement-images-and-files.component';
import { Router } from '@angular/router';
import { ServerApiInterfaceServiceService } from 'src/app/services/server-api-interface-service.service';
import { SessionStorageService } from 'src/app/services/session-storage.service';
import { TYPED_NULL_EXPR } from '@angular/compiler/src/output/output_ast';
import { ViewPermitInspectionComponent } from '../../widget-app/view-permit-inspection/view-permit-inspection.component';
...
export class PermitViewComponent extends WidgetPageBase implements OnInit {
  ...
  progresstoPAFormGroup: FormGroup;
  ...
  permit_alteration_ref_no:string;
  ...
  createFormGroups() {
    ...
    this.progresstoPAFormGroup = this.fbProgresstoPA.group({
      'proposedStartDatePA': ['', [Validators.required]],
      'proposedEndDatePA': ['', [Validators.required]]
    });
    // progresstoPA
  ...
  onChangeRequest() {
    ...
    applicationId = this.globalParameters.get(GLOBAL_PARAM_KEY.PERMIT_FORM)?.applicationId;
    if (applicationId === undefined) {
      applicationId = this.globalParameters.get("applicationId")
  ...
  onAppDialogClose(data) {
    this.progresstoPAFormGroup.reset();
  ...
  page_onEditChangeReq(data) {
    ...
    this.globalParameters.set(GLOBAL_PARAM_KEY.PERMIT_FORM, data.dataContext);
  }

  page_onTrackChangeReqChanges(data) {
console.log("data",data)
console.log("data on track changes",
this.globalParameters.get(GLOBAL_PARAM_KEY.VIEW_PERMIT_FORM)
)

let temp=this.globalParameters.get(GLOBAL_PARAM_KEY.VIEW_PERMIT_FORM);
this.permit_alteration_ref_no = data.dataContext.CRReferenceNumber;
this.assessment_status = data.dataContext.ChangeRequestStatus;
this.assessment_comments = data.dataContext.assessment_comments;
  ...
  page_onProgressToPA() {
    ...
    // proposed_start_date: this.progresstoPAFormGroup.controls.proposedStartDatePA.value,
    let permitFormData = this.globalParameters.get(GLOBAL_PARAM_KEY.VIEW_PERMIT_FORM);

    this.progresstoPAFormGroup.controls.proposedStartDatePA.setValue(permitFormData?.proposed_start_date);
    this.progresstoPAFormGroup.controls.proposedEndDatePA.setValue(permitFormData?.proposed_end_date);
  ...
  onConfirmProgressToPA() {
    ...
    this.progresstoPAFormGroup.markAllAsTouched();
    if (this.progresstoPAFormGroup.valid) {
      // this.trigger.closeMenu();
      let reqBody = {
        // action: 'start',
        proposed_start_date: this.progresstoPAFormGroup.controls.proposedStartDatePA.value,
        proposed_end_date: this.progresstoPAFormGroup.controls.proposedEndDatePA.value
      }
      this.progresstoPAFormGroup.reset();
      // let applicationId = this.globalParameters.get("applicationId");
      // this.pageAPIMethodPut(`/api/v1/applications/action/${applicationId}`, reqBody).then(resp => {
      ...
      this.pageAPIMethodPost(`/api/v1/applications/action/${appId}/progress-to-pa`, reqBody).then(resp => {
        this.onAppDialogClose(null);
        this.page_refreshData();
  ...
  getPermitRequestBody(globalParamsMode) {
    ...
    data.secondary_contact_email = userInfo.contact_email_id;
  ...
  onAddNewReinstatementBtnClick() {
    ...
    this.createReinstatementFormDataModel.globalParameters.set(GLOBAL_PARAM_KEY.PERMIT_FORM, this.globalParameters.get(GLOBAL_PARAM_KEY.VIEW_PERMIT_FORM));

    this.createReinstatementFormDataModel.globalParameters.set(GLOBAL_PARAM_KEY.REINSTATEMENT_FORM, null);

console.log("reinstatement response",this.globalParameters.get(GLOBAL_PARAM_KEY.VIEW_PERMIT_FORM))
  ...
  page_onEditReinstateMentClick(eventparams, wigDataContext, params) {
    ...
    this.createReinstatementFormDataModel.globalParameters.set('mode', FormModeConstant.EDIT);
      ...
      response => {
        try {
          ...
          if (response) {
            ...
            this.createReinstatementFormDataModel.mode=FormModeConstant.EDIT;
