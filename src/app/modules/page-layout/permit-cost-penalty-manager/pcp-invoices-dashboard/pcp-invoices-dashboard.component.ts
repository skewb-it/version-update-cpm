...
import {
  ...
  InvoicesListRequestModel,
} from 'src/app/modules/widget-app/cost-penalty-manager/invoices-list/invoices-list.model';
import {
  MASTER_DATA,
  MASTER_DATA_DOMAIN,
...
import { InvoicesListComponent } from 'src/app/modules/widget-app/cost-penalty-manager/invoices-list/invoices-list.component';
...
import { PermitNavigationDataModel } from './pcp-invoices-dashboard.model';
...
@Component({
  selector: 'app-pcp-invoices-dashboard',
  templateUrl: './pcp-invoices-dashboard.component.html',
  styleUrls: ['./pcp-invoices-dashboard.component.css'],
})
export class PcpInvoicesDashboardComponent
  ...
  implements OnInit
{
  ...
  invoicesListDataModel: InvoicesListDataModel;
  invoicesListConfigModel: InvoicesListConfigModel;
  ...
  constructor(
    ...
    this.invoicesListDataModel = InvoicesListDataModel.getInstance();
    this.invoicesListConfigModel = InvoicesListConfigModel.getInstance();
  ...
  ngOnInit(): void {
    ...
    this.bindInvoicesListDataModel();
    this.bindInvoicesListEvents();
  ...
  setComponentRefMap() {
    this.compRefInstancesMap.set('permitList', this.invoicesListComponent);
  ...
  checkAndBindJourney(journeyType) {
    this.invoicesListDataModel.apireqdata = new InvoicesListRequestModel();
    this.invoicesListDataModel.journeyType = journeyType;
    this.invoicesListComponent.resetPagination();
  ...
  checkAndBindDynamicFilterData(shouldCompulsoryRefreshList) {
    ...
    if (
      ...
      this.invoicesListDataModel.apireqdata = new InvoicesListRequestModel();
      this.invoicesListDataModel.filterType = FILTER_TYPE.DYNAMIC_FILTER;
      this.invoicesListDataModel.apireqdata = {
        ...filterGlobalPersistData.persistantDynamicFilterModel,
      };
      this.invoicesListComponent.resetPagination();
      this.invoicesListConfigModel.CallerToComp.emit(
  ...
  bindPCPFilterDataModel() {
    // abstract methid for setGlobalParams
    super.setGlobalParams(this.invoicesListDataModel);
  }
  ...
  bindInvoicesListDataModel() {
    // abstract methid for setGlobalParams
    super.setGlobalParams(this.invoicesListDataModel);
    this.invoicesListDataModel.columnsToDisplay = [
    ...
    this.invoicesListDataModel.apiUrls.advanceFilter = '/api/v1/applications';
    this.invoicesListDataModel.apiUrls.quickFilter =
      '/api/v1/applications/quick-filter';
    this.invoicesListDataModel.apiUrls.dynamicFilter = '/api/v1/applications';
  }
  ...
  bindInvoicesListEvents() {
    ...
    super.addEventListener(
      eventActions,
      this.invoicesListDataModel,
      this.invoicesListConfigModel
    );
  }
